...
const Game = () => {
    ...
    const handleTouch = (e: React.TouchEvent) => {
        ...
        const x = touch.clientX - rect.left - PLAYER_SIZE / 2;
        setPlayerPosition(Math.max(0, Math.min(GAME_WIDTH - PLAYER_SIZE, x)));
    };
    ...
    return (
        <div className="relative">
            <motion.div variants={textVariant(0)}>
                <p className="sm:text-[18px] text-[14px] text-secondary uppercase tracking-[4px] font-medium text-center sm:text-left">Mini Lab</p>
                <h2 className="text-white font-black md:text-[60px] sm:text-[50px] xs:text-[40px] text-[30px] leading-tight text-center sm:text-left">
            ...
            <div className="mt-10 flex flex-col items-center">
                ...
                <div className="scale-[0.75] xs:scale-[0.85] sm:scale-100 origin-center">
                    <div
                        ref={gameRef}
                        onTouchMove={handleTouch}
                        onTouchStart={handleTouch}
                        className="relative bg-tertiary/40 backdrop-blur-md border border-white/5 rounded-3xl overflow-hidden shadow-2xl cursor-none touch-none"
                    ...
                    >
                        ...
                        <AnimatePresence>
                            ...
                            {gameState === "GAMEOVER" && (
                                ...
                                >
                                    ...
                                    <h3 className="text-white text-3xl font-black mb-2 text-center">CRITICAL FAILURE</h3>
                        ...
                        {/* Score UI */}
                        <div className="absolute top-6 left-6 right-6 flex justify-between z-10 font-bold">
                            <div className="bg-primary/60 backdrop-blur-md px-4 py-2 rounded-xl border border-white/5">
                                <p className="text-secondary text-[10px] uppercase tracking-widest leading-none mb-1 text-center">Score</p>
                                <p className="text-white text-lg text-center">{score}</p>
                            </div>
                            <div className="bg-primary/60 backdrop-blur-md px-4 py-2 rounded-xl border border-white/5">
                                <p className="text-secondary text-[10px] uppercase tracking-widest leading-none mb-1 text-center">High</p>
                                <p className="text-white text-lg text-center">{highScore}</p>
                            </div>
                        </div>
                ...
                <div className="mt-4 text-secondary text-xs font-medium flex flex-col sm:flex-row gap-4 sm:gap-8 items-center bg-tertiary/20 px-8 py-4 rounded-2xl border border-white/5 w-full max-w-[400px]">
                    <div className="flex items-center gap-2">
                        <div className="hidden sm:flex gap-2">
                            <div className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center font-bold">←</div>
                            <div className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center font-bold">→</div>
                        </div>
                        <span className="text-center sm:text-left">Drag, Tap or use arrow keys to Move Paddle</span>
                    </div>
                </div>
            </div>
        </div>
